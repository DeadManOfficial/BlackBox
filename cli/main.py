...
try:
    from .intel_commands import register_intel_commands
    HAS_INTEL = True
except ImportError:
    HAS_INTEL = False
...
if HAS_INTEL:
    register_intel_commands(app)

def version_callback(value: bool):
    if value:
        from modules.scraper import __version__
...
def scrape(
    ...
    async def _scrape():
        from modules.scraper import Engine, Config
    ...
    async def _search():
        ...
        async with Engine(config) as engine:
            if scrape_top > 0:
                ...
                from modules.scraper.discovery.aggregator import SearchAggregator
...
def tor(action: str = typer.Argument("status", help="Action: start/stop/restart/status")):
    ...
    async def _tor():
        from modules.scraper.core.config import Config
        from modules.scraper.fetch.tor_manager import TORManager
        config = Config.from_env()
...
def config(
    ...
    from modules.scraper.core.config import Config
    import yaml
    # Use absolute path relative to this file (security fix)
...
def stats():
    ...
    async def _stats():
        from modules.scraper.core.config import Config
        from modules.scraper.ai.llm_router import FreeLLMRouter
        cfg = Config.from_env()
        cfg.load_api_keys()
