...
from .scheduler import Priority, ScheduledRequest, Scheduler
...
class Engine:
    ...
    async def search_and_scrape(
        ...
        *,
        ...
        from ..discovery.aggregator import SearchAggregator
        ...
        if filter_llm:
            from ..ai.relevance import filter_relevant
    ...
    async def _init_components(self) -> None:
        ...
        if self.config.tor.enabled:
            from ..fetch.tor_manager import TORManager

            self._tor_manager = TORManager(self.config.tor)
            await self._tor_manager.start()

        # Initialize LLM router
        from ..ai.llm_router import FreeLLMRouter
    ...
    async def _process_request(self, request: ScheduledRequest) -> ScrapeResult:
        ...
        from ..fetch.downloader import AdaptiveDownloader
        from ..fetch.proxy_manager import ProxyManager
